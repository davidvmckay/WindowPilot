...
- `scripts/release.sh <ver>`: build → assemble bundle → sign inner-first → DMG → notarize → staple → appcast → GitHub release → push appcast.xml. Order is load-bearing: the release asset must exist BEFORE the appcast goes live.
