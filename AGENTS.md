...
- Preflight (before build): refuses to run unless on branch `main` with a clean tracked tree (staged + unstaged; untracked ignored) — so the local DRY_RUN test below also needs a clean main to run.
